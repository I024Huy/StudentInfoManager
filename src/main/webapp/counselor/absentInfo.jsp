...
<body><div id="particles-js"><canvas class="particles-js-canvas-el" width="1322" height="774" style="width: 100%; height: 100%;"></canvas></div>
	...
	<div class="container" style="max-width: 1500px">
		<div class="row">
			<div class="col-md-12">
				<h5>学生缺旷情况管理</h5>
				<form name="studentquery" action="/StudentInfo/CounselorHandler/keyQueryAbsent/1" method="post">
					...
					<div class="row">
						...
						<div class="col-md-1 pos">
							<div style="float: right">
								<a class="btn btn-primary" href="/StudentInfo/CounselorHandler/exportStudentExcel/absent">导出</a>
							</div>
						</div>
					</div>
				</form>
					...
					<div class="col-md-12">
						<div class="margin:0 15px">
							<table style="overflow: scroll;"
								...
								<c:forEach var="absent" items="${pageInfo.list}"
									varStatus="loop">
									<tr>
										<td>${absent.student.username }</td>
										<td>${absent.student.name }</td>
										<td>
											<c:if test="${absent.student.gender == 1}">
												男
											</c:if>
											<c:if test="${absent.student.gender == 0}">
												女
											</c:if>
										</td>
										<td>${absent.course }</td>
										<td>
											<fmt:formatDate value="${absent.date}" pattern="yyyy-MM-dd"/>
										</td>
										<td>${absent.content }</td>
										<td>${absent.student.myclass }</td>
										<td>${absent.student.grade }</td>
										<td>${absent.student.profession.name }</td>
										<td>${absent.student.college.name }</td>
										<td>${absent.student.room }</td>
										<td>
											<button value="删除" onclick="deleteStudentAbsent('${absent.kid}')" class="btn btn-danger" >删除</button>
										</td>
									</tr>
								</c:forEach>
							</table>
	...
	<div class="row">
		...
		<div class="col-md-4">
			...
			<ul class="pagination" style="background: darkgray">
				<li><a href="/StudentInfo/CounselorHandler/absentInfo/1">首页</a></li>
				<c:if test="${pageInfo.hasPreviousPage }">
					<li><a href="/StudentInfo/CounselorHandler/absentInfo/${pageInfo.pageNum-1}"
				...
				<c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
					...
					<c:if test="${page_Num != pageInfo.pageNum }">
						<li><a href="/StudentInfo/CounselorHandler/absentInfo/${page_Num }">${page_Num }</a></li>
					</c:if>
				...
				<li><a href="/StudentInfo/CounselorHandler/absentInfo/${pageInfo.pages}">末页</a></li>
			</ul>
			</nav>
		</div>
	</div>
...
<script>
	...
	function deleteStudentAbsent(kid) {
		const flag = window.confirm("您确定要删除吗？")
		if(flag){
			window.location.href = "/StudentInfo/CounselorHandler/deleteStudentAbsent/"+kid;
		}
	}
</script>
